  ...
  override def receiveCommand: Receive = {
    ...
    case AccountActor.ConfirmMoneyFrozenFail(deliveryId, reason) =>
      persist(FreezingMoneyFailed(deliveryId, reason)) { e =>
        updateState(e)
        log.info(s"unable to finish sagas.transaction ${state.transactionId}, reason: ${state.failureReason}")
        context.system.eventStream.publish(s"unable to finish sagas.transaction ${state.transactionId}, reason: ${state.failureReason}")
      }
    ...
    case AccountActor.ConfirmTransactionFinished(deliveryId) =>
      persist(TransactionFinished(deliveryId)) { e =>
        updateState(e)
        log.info(s"sagas.transaction ${state.transactionId} finished successfully")
        context.system.eventStream.publish(s"sagas.transaction ${state.transactionId} finished successfully")
      }
